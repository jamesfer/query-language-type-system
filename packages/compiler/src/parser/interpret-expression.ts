...
type InterpreterFunction<T> = (tokens: ExpressionToken[], previous: Expression | undefined, precedence: Precedence) => Free<WithMessages<WithTokens<T>[]>>
...
function doWithFreeState<T>(f: (state: MessageState) => Free<T>): Free<WithMessages<T>> {
  const state = new MessageState();
